<div>
    @if($object?->activities()->whereNot('subject_event', 'created')->count())
        <div class="flex flex-col gap-2 p-3 border rounded">
            <h3 class="font-bold text-black mb-2 flex flex-wrap gap-2 justify-between">
                ...
                <a href="{{ route('user.activity', ['subject_type' => $object->getMorphClass(), 'subject_id' => $object->getKey()]) }}"
                   target="_blank"
                   class="text-indigo-400 hover:text-indigo-700 text-xs">

                    {{ __('View the entire history') }}
                </a>
            </h3>
