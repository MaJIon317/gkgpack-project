    ...
    <div x-cloak :class="sidebarOpen ? 'translate-x-0 ease-out' : '-translate-x-full ease-in'" class="fixed h-full inset-y-0 left-0 z-30 w-64 overflow-y-auto transition duration-300 transform bg-gray-900 lg:translate-x-0 lg:static lg:inset-0">
        ...
        <nav class="mt-10">
            ...
            @foreach ($sidebars as $sidebar)
                <a class="flex items-center px-6 py-2 mt-4 {{ $sidebar->get('route') == $current ? 'text-gray-100 bg-gray-700 bg-opacity-25' : 'text-gray-500 hover:bg-gray-700 hover:bg-opacity-25 hover:text-gray-100' }}" href="{{ $sidebar->get('link') }}" wire:navigate>
                    <span class="mx-3">{{ $sidebar->get('name') }}</span>
                </a>
            @endforeach
        </nav>
