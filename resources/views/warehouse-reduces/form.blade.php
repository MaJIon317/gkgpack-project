<x-form submit="{{ $warehouseReduce ? 'update' : 'store' }}" class="col-span-5 xl:col-span-3">

    <x-slot name="title">
        {{ $warehouseReduce ? __('Edit warehouse reduce') : __('Create warehouse reduce') }}
    </x-slot>
            ...
            <x-table-default>
                ...
                @foreach($products as $product)
                    <tr>
                        ...
                        <x-table-default-column>
                            {{ $product->stocks()->where('warehouse_id', $warehouse_id)->where(function ($query) use ($warehouseReduce) {
                                if ($warehouseReduce) {
