<div>
    <x-slot:actions>
        <x-button
            message="{{ __('Create warehouse reduce') }}"
            x-on:click="Livewire.dispatch('openModal', {component: 'warehouse-reduce-form'})"
        />
    </x-slot:actions>
    ...
    <x-table-default>
        ...
        @foreach($warehouseReduces as $warehouseReduce)
            <tr class="{{ $warehouseReduce->warehouse->trashed() ? 'opacity-25' : 'opacity-100' }}">
                <x-table-default-column>{{ $warehouseReduce->warehouse->name }}</x-table-default-column>
                <x-table-default-column>{{ $warehouseReduce->note }}</x-table-default-column>
                <x-table-default-column>{{ $warehouseReduce->created_at->diffForHumans() }}</x-table-default-column>
                <x-table-default-column>{{ $warehouseReduce->updated_at->diffForHumans() }}</x-table-default-column>
                <x-table-default-column class="text-right">
                    <x-button
                        color="a"
                        title="{{ __('Edit') }}"
                        wire:click="$dispatch('openModal', { component: 'warehouseReduce-form', arguments: { warehouseReduce: {{ $warehouseReduce }} }})"
                    >
                    ...
                    <x-button
                        color="a"
                        title="{{ __('Delete') }}"
                        wire:click="delete({{ $warehouseReduce }})"
