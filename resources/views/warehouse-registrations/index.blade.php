<div>
    <x-slot:actions>
        <x-button
            message="{{ __('Create warehouse registration') }}"
            x-on:click="Livewire.dispatch('openModal', {component: 'warehouse-registration-form'})"
        />
    </x-slot:actions>

    <x-table-default>
        <x-slot name="thead">
            <x-table-default-column tag="th">{{ __('Warehouse') }}</x-table-default-column>
        ...
        @foreach($warehouseRegistrations as $warehouseRegistration)
            <tr class="{{ $warehouseRegistration->warehouse->trashed() ? 'opacity-25' : 'opacity-100' }}">
                <x-table-default-column>{{ $warehouseRegistration->warehouse->name }}</x-table-default-column>
                <x-table-default-column>{{ $warehouseRegistration->note }}</x-table-default-column>
                <x-table-default-column>{{ $warehouseRegistration->created_at->diffForHumans() }}</x-table-default-column>
                <x-table-default-column>{{ $warehouseRegistration->updated_at->diffForHumans() }}</x-table-default-column>
                <x-table-default-column class="text-right">
                    <x-button
                        color="a"
                        title="{{ __('Edit') }}"
                        wire:click="$dispatch('openModal', { component: 'warehouseRegistration-form', arguments: { warehouseRegistration: {{ $warehouseRegistration }} }})"
                    >
                    ...
                    <x-button
                        color="a"
                        title="{{ __('Delete') }}"
                        wire:click="delete({{ $warehouseRegistration }})"
