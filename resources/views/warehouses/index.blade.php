<div>
    <x-slot:actions>
        <x-button
            message="{{ __('Create warehouse') }}"
            x-on:click="Livewire.dispatch('openModal', {component: 'warehouse-form'})"
        />
    </x-slot:actions>
    ...
    <x-table-default>
        ...
        @foreach($warehouses as $warehouse)
            <tr>
                <x-table-default-column>{{ $warehouse->id }}</x-table-default-column>
                <x-table-default-column>{{ $warehouse->name }}</x-table-default-column>
                <x-table-default-column>{{ $warehouse->created_at->diffForHumans() }}</x-table-default-column>
                <x-table-default-column>{{ $warehouse->updated_at->diffForHumans() }}</x-table-default-column>
                <x-table-default-column class="text-right">
                    <x-button
                        color="a"
                        message="{{ __('Add leftovers') }}"
                        wire:click="$dispatch('openModal', { component: 'warehouse-registration-form', arguments: { warehouse_id: {{ $warehouse->id }} }})"
                    />

                    <x-button
                        color="a"
                        title="{{ __('Edit') }}"
                        wire:click="$dispatch('openModal', { component: 'warehouse-form', arguments: { warehouse: {{ $warehouse }} }})"
                    >
