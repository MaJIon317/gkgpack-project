<div>
    <x-slot:actions>
        ...
        <x-button
            message="{{ __('Create product') }}"
            x-on:click="Livewire.dispatch('openModal', {component: 'product-form'})"
        />
    ...
    <x-table-default>
        <x-slot name="thead">
            <x-table-default-column tag="th">{{ __('Image') }}</x-table-default-column>
            ...
            <x-table-default-column tag="th">{{ __('Serction') }}</x-table-default-column>
            <x-table-default-column tag="th">{{ __('Stock') }}</x-table-default-column>
            <x-table-default-column tag="th">{{ __('Currect stock') }}</x-table-default-column>
                    ...
                    <div x-cloak x-show="actionOpen" class="absolute right-0 z-10 mt-2 overflow-hidden bg-white rounded-lg shadow-xl w-40">
                        <div class="z-10 bg-white divide-y divide-gray-100 rounded-lg shadow-sm w-44 dark:bg-gray-700 dark:divide-gray-600 block">
                            <x-button
                                ...
                                x-on:click="Livewire.dispatch('openModal', {component: 'import', arguments: { key: 'product' }})"
                            />

                            <x-button
                                class="flex items-center px-3 py-2 text-gray-600 hover:text-white hover:bg-indigo-600 w-full"
                                message="{{ __('Upload Images') }}"
                                x-on:click="Livewire.dispatch('openModal', {component: 'import', arguments: { key: 'product-image' }})"
                            />
                        </div>
                    </div>
                </div>
        ...
        @foreach($products as $product)
            <tr>
                ...
                <x-table-default-column>{{ $product->name }}</x-table-default-column>
                <x-table-default-column>{{ $product->barcode }}</x-table-default-column>
                <x-table-default-column>{{ $product->section }}</x-table-default-column>
                <x-table-default-column>{{ $product->stocks()->where('reserve', false)->sum('qty') }}</x-table-default-column>
                <x-table-default-column>{{ $product->stocks()->sum('qty') }}</x-table-default-column>
        
                <x-table-default-column>{{ \Carbon\Carbon::parse( $product->updated_at )->setTimezone( get_user_timezone())->diffForHumans() }}</x-table-default-column>
                <x-table-default-column class="text-right" colspan="2">
