    ...
    @if ($order)
        <x-table-default-column>
            @if (!$order->user_id)
                <x-button
                    size="sm"
                    message="{{ __('To take on') }}"
                    wire:click="take({{ $order }})"
                />
            @else
                <x-button
                    size="sm"
                    color="{{ $order->status == 'completed' ? 'dark' : 'default' }}"
                    message="{{ $order->status == 'completed' ? __('Completed') : __('To manage') }}"
                    wire:click="$dispatch('openModal', { component: 'order-collect', arguments: { order: {{ $order }} }})"
                />
            @endif
        </x-table-default-column>
        <x-table-default-column>{{ $order->name }}</x-table-default-column>
        <x-table-default-column>{{ $order->barcode }}</x-table-default-column>
        <x-table-default-column>{{ $order->note }}</x-table-default-column>
        <x-table-default-column>{{ $order->status }}</x-table-default-column>
        <x-table-default-column>{{ \Carbon\Carbon::parse( $order->created_at )->setTimezone( get_user_timezone() )->diffForHumans() }}</x-table-default-column>
        <x-table-default-column>{{ \Carbon\Carbon::parse( $order->updated_at)->setTimezone( get_user_timezone() )->diffForHumans() }}</x-table-default-column>
        <x-table-default-column class="text-right" colspan="2">
            @if (auth()->user()->hasRole(['admin', 'user']))
                <x-button
                    color="a"
                    title="{{ __('Edit') }}"
                    wire:click="$dispatch('openModal', { component: 'order-form', arguments: { order: {{ $order }} }})"
                >
