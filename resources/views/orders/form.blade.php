        ...
        <x-slot name="form">
            ...
            @if ($products->count())
                <x-table-default>
                    <x-slot name="thead">
                        ...
                        <x-table-default-column tag="th" class="w-[100px]">{{ __('Qty') }}</x-table-default-column>
                        <x-table-default-column tag="th" class="w-[100px]">{{ __('Stock') }}</x-table-default-column>
                    ...
                    @foreach ($products as $product)
                        <tr>
                            ...
                            <x-table-default-column>
                                <x-fields.select
                                    classBlock=""
                                    wire:model.live="data.{{ $product->id }}.warehouse_id"
                                    required>
                            ...
                            <x-table-default-column>
                                ...
                                @endphp
                                {{ $product->stocks()->where('warehouse_id', $data[$product->id]['warehouse_id'] ?? 0)->where(function ($query) use ($order_product_id) {
            ...
            <x-fields.select
                ...
                @foreach (\App\Models\User::all() as $user)
                    <option value="{{ $user->id }}" @if ($user_id == $user->value) selected @endif>{{ __(ucfirst($user->name)) }}</option>
                @endforeach
            </x-fields.select >
