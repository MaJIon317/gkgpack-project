<x-form submit="{{ $warehouseMoving ? 'update' : 'store' }}" class="col-span-5 xl:col-span-3">

    <x-slot name="title">
        {{ $warehouseMoving ? __('Edit warehouse moving') : __('Create warehouse moving') }}
    </x-slot>
    ...
    <x-slot name="form">
        ...
        <div class="grid md:grid-cols-2 grid-cols-1 gap-x-8">
            <x-fields.select
                label="{{ __('From the warehouse') }}"
                wire:model="from_warehouse_id"
                required>

                @foreach ($warehouses as $warehouse)
                    <option value="{{ $warehouse->id }}" {{ $from_warehouse_id == $warehouse->id  ? 'selected' : '' }}>{{ __($warehouse->name) }}</option>
                @endforeach
            </x-fields.select>

            <x-fields.select
                label="{{ __('	To the warehouse') }}"
                wire:model="to_warehouse_id"
                required>

                @foreach ($warehouses as $warehouse)
                    <option value="{{ $warehouse->id }}" {{ $to_warehouse_id == $warehouse->id  ? 'selected' : '' }}>{{ __($warehouse->name) }}</option>
                @endforeach
            </x-fields.select>
        </div>
        ...
        @if ($products->count())
            <x-table-default>
                <x-slot name="thead">
                    ...
                    <x-table-default-column tag="th" class="min-w-[100px]">{{ __('Remains (from the warehouse)') }}</x-table-default-column>
                    <x-table-default-column tag="th" class="min-w-[100px]">{{ __('Remains (to the warehouse)') }}</x-table-default-column>
                ...
                @foreach($products as $product)
                    <tr>
                        ...
                        <x-table-default-column>
                            {{ $product->stocks()->where('warehouse_id', $from_warehouse_id)->where(function ($query) use ($warehouseMoving) {
                                if ($warehouseMoving) {
                        ...
                        <x-table-default-column>
                            {{ $product->stocks()->where('warehouse_id', $to_warehouse_id)->where(function ($query) use ($warehouseMoving) {
                                if ($warehouseMoving) {
                                    $query->whereNot('warehouse_moving_id', $warehouseMoving->id)
                                        ->orWhere('warehouse_moving_id', null);
                                }
