<div>
    <x-slot:actions>
        <x-button
            message="{{ __('Create a moving') }}"
            x-on:click="Livewire.dispatch('openModal', {component: 'warehouse-moving-form'})"
        />
    </x-slot:actions>

    <x-table-default>
        <x-slot name="thead">
            <x-table-default-column tag="th">{{ __('From the warehouse') }}</x-table-default-column>
            <x-table-default-column tag="th">{{ __('To the warehouse') }}</x-table-default-column>
        ...
        @foreach($warehouseMovings as $warehouseMoving)
            <tr class="{{ ($warehouseRegistration->from_warehouse->trashed() || $warehouseRegistration->to_warehouse->trashed()) ? 'opacity-25' : 'opacity-100' }}">
                <x-table-default-column>{{ $warehouseMoving->from_warehouse->name }}</x-table-default-column>
                <x-table-default-column>{{ $warehouseMoving->to_warehouse->name }}</x-table-default-column>
                <x-table-default-column>{{ $warehouseMoving->note }}</x-table-default-column>
                <x-table-default-column>{{ $warehouseMoving->created_at->diffForHumans() }}</x-table-default-column>
                <x-table-default-column class="text-right">
                    <x-button
                        color="a"
                        title="{{ __('Edit') }}"
                        wire:click="$dispatch('openModal', { component: 'warehouse-moving-form', arguments: { warehouseMoving: {{ $warehouseMoving }} }})"
                    >
                    ...
                    <x-button
                        color="a"
                        title="{{ __('Delete') }}"
                        wire:click="delete({{ $warehouseMoving }})"
